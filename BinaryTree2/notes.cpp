...
public:
    ...
    int hight(Node *node){
        ...
        int left_height = hight(node->left);
        int right_height = hight(node->right);
        int height = max(left_height , right_height);
        return height + 1;
    }
    // No of Nodes In the longest path between in tow leaves
    int diameter_of_tree(Node *root){
        if(root == NULL){
            return 0;
        }
        int left_dimeter = diameter_of_tree(root->left);
        int leftHt = hight(root->left);

        int right_dimeter = diameter_of_tree(root->right);
        int rightHt = hight(root->right);

        int self_dia = leftHt + rightHt + 1;

        return max(self_dia , max(right_dimeter , left_dimeter));
    }
    //******************************************************************************************************/
    /**
     * Diameter Approch 2
    ...
    {
        if (root == NULL && sub_tree == NULL) return true;
        else if (root == NULL || sub_tree == NULL || root->data != sub_tree->data)  return false;

        if (!isIdentical(root->left, sub_tree->left)) return false;
        if (!isIdentical(root->right, sub_tree->right)) return false;
        return true;
    }
    ...
    bool issubtree(Node *root, Node *sub_tree)
    {
        ...
        if (root->data == sub_tree->data) return isIdentical(root, sub_tree);
...
int main(){
    ...
    cout<<"The Diameter Of Tree Approch 1 : "<<tree.diameter_of_tree(root)<<endl;

    cout<<"The Diameter Of Tree Approch 2 : "<<tree.diameter_of_tree_2(root).diameter<<endl;
}