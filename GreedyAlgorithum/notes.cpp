//              Introduction To Greddy Algorithm
/**
 * Optimization -> min , max
 * No Fixed Rule
 * Not Realize
 * 
 *** Greedy Alogorithm Is the porblem solving technique where we make the locally optimum choice at each stage and hope to achieve a global optimum
*/
...
/**
 * Fractional Knapsack Problem
 * Given The Weight and Vlaues of the items and the capacity of the knapsack, find the maximum value that can be put in a knapsack of given capacity.
 * value[] = {60, 100, 120}
 * weight[] = {10, 20, 30}
 * capacity = 50
 * Output: 240
 ...
 * Givend two array A and B of size N and M respectively(both N and M are same size), find the pair (a, b) such that a belongs to A and b belongs to B and the absolute difference between a and b is minimum.
...
int max_len_chair_pair(vector<pair<int , int>> &p){
    int result = 1;
    int n = p.size();
...
int main(){
    ...
    int result = max_len_chair_pair(pair);
